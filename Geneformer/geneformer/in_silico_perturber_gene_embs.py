...
def load_and_filter(filter_data, nproc, input_data_file, seed):
...
def quant_cos_sims(model, 
        ...
        if perturb_group == True:
            ...
            if perturb_type == 'overexpress': #added this line of code as the indices should only be removed in overexpression
                ...
                original_minibatch_emb = remove_indices_from_emb_batch(original_minibatch_emb, 
                                                                      ...
                                                                      gene_dim=1)
            '''
            #may need to modify this if overexpressing - see above - ensures same dimension of original and perturbed embeddings.
            if original_minibatch_emb.size() != minibatch_emb.size():
                original_minibatch_emb = remove_indices_from_emb_batch(original_minibatch_emb, 
                                                                  indices_to_perturb_minibatch, 
                                                                  gene_dim=1)
    ...
    if max_ncells != 1:
        perturb_emb = perturb_emb.reshape(-1, perturb_emb.shape[-1])
        original_emb = original_emb.reshape(-1,original_emb.shape[-1])
    token_list = [token for token in downsampled_data['input_ids'][0] if token not in tokens_to_perturb]
    
    if len(perturb_emb) < len(token_list):
        token_list.pop()
    ...
    try:
        euclideans_df['token'] = token_list
    except ValueError:
        token_list.append('-1')
        euclideans_df['token'] = token_list
...
class InSilicoPerturberGeneEmbs:
    valid_option_dict = {
    ...
    def __init__(
        ...
        sample_cells : None, int
            Number of cells to filter from the dataset used.
            If None, will filter all cells.
        seed: int
    ...
    def perturb_data(self, 
        ...
        """
        filtered_input_data = load_and_filter(self.filter_data, self.nproc, input_data_file, self.seed)
    ...
    def in_silico_perturb(self,
        ...
        if (self.tokens_to_perturb != "all") and (self.perturb_type != "overexpress"):
            ...
            if self.sample_cells:
                filtered_input_data = filtered_input_data.select([i for i in range(self.sample_cells)])
            filtered_input_data = filtered_input_data.filter(if_has_tokens_to_perturb, num_proc=self.nproc)
            if len(filtered_input_data) == 0:
                logger.error(
