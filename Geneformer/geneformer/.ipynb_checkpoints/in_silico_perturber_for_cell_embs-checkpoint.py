...
def quant_cos_sims(model, 
    ...
    cos_sims_perturb_vs_original=cos(perturb_emb, original_emb)
    cos_sims_df = label_embs(cos_sims_perturb_vs_original,downsampled_data,emb_label)
    cos_sims_df = cos_sims_df.rename(columns={0:"cosine_similarity"})
    cos_sims_output_path = (Path(f'{output_prefix}_cos_sims')).with_suffix(".csv")
    cos_sims_df.to_csv(cos_sims_output_path)
...
class InSilicoPerturberEmbs:
    ...
    def __init__(
        ...
        if self.filter_state not in self.filter_data.values():
            logger.error(f'Cannot filter by {self.filter_state} as it is not in possible states')
    ...
    def perturb_data(self, 
        ...
        if self.cell_states_to_model is None:
            ...
            def filter_for_origin(example):
                return example[state_name] in [state_to_filter]
