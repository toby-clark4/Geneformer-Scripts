...
def draw_arrows(adata,n,label):
    for i in range(n):
        point1 = adata.obsm['X_umap'][i]  # UMAP coordinates for data point 0
        point2 = adata.obsm['X_umap'][i+n]  # UMAP coordinates for data point 10
        ...
        colour_dict={"nf original":"gold","dcm original":"cyan","hcm original":"pink","nf perturbed":"gray","dcm perturbed":"orange","hcm perturbed":"lime"}
        plt.plot(xvals, yvals, linestyle='-',color=colour_dict[label[i]])
        plt.scatter(xvals[0], yvals[0], color=colour_dict[label[i]], s=50)
        plt.scatter(xvals[1], yvals[1], color=colour_dict[label[i+n]], s=50)
    handles = [plt.Line2D([0], [0], marker='o', color=color, markerfacecolor=color, markersize=10, label=label)
          for label,color in colour_dict.items()]  
    #plt.legend(handles,loc='best')
    plt.legend(handles,colour_dict.keys(),loc='best',fontsize='xx-small')
    plt.show()
    
    
def plot_umap(embs_df, emb_dims, label, output_file, kwargs_dict, arrows_output, add_arrows=False):
    ...
    template_adata=sc.read("/mnt/scratchc/ghlab/toby/in_silico_perturb_data/template_adata/original_144k.h5ad")
    adata = anndata.AnnData(X=only_embs_df, obs=obs_dict, var=vars_dict)
    sc.tl.pca(adata, svd_solver='arpack')
    sc.pp.neighbors(adata)
    sc.tl.umap(adata, init_pos=template_adata.obsm['X_umap'])
    adata = sc.pp.neighbors_umap(adata, adata=template_adata, copy=True)
    #adata.write_h5ad("/mnt/scratchc/ghlab/toby/in_silico_perturb_data/template_adata/original_144k.h5ad")
    ...
    if add_arrows == True:
        fig = draw_arrows(adata,int(len(embs_df)/2),embs_df[label])
        plt.savefig(arrows_output)
    ...
    def plot_embs(self,
        ...
        if plot_style == "umap":
            ...
            for label in self.labels_to_plot:
                ...
                arrows_prefix_label = output_prefix_label + f"_arrow"
                output_file = (Path(output_directory) / output_prefix_label).with_suffix(".pdf")
                arrows_output = (Path(output_directory) / arrows_prefix_label).with_suffix(".pdf")
                plot_umap(embs, emb_dims, label, output_file, kwargs_dict, arrows_output, add_arrows)
