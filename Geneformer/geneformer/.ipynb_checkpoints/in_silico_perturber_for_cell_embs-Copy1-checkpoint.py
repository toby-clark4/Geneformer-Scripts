...
def label_embs(embs, downsampled_data, emb_labels):
    ...
    print(f'{embs_df.shape} is embs_df shape')
    print(f'{downsampled_data.shape} is downsampled_data shape')
    if emb_labels is not None:
        for label in emb_labels:
            emb_label = downsampled_data[label]
            print(f'{len(emb_label)} is emb_label')
            ...
            for state in possible_states:
                if perturb_group == False:
                    cos_sims_vs_alt_dict[state] += cos_sim_shift(original_emb, 
                                                                ...
                                                                output_directory,
                                                                output_prefix,
                                                                downsampled_data[i:i+forward_batch_size,:],
                                                                emb_label,
                                                                state,
                                                                possible_states)
                elif perturb_group == True:
...
def cos_sim_shift(original_emb, 
    ...
    if state == possible_states[len(possible_states)-1]:
