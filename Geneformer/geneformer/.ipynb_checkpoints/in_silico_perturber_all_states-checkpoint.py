...
from .in_silico_perturber_for_cell_embs import label_embs
...
def downsample_and_sort(data_shuffled, max_ncells):
    ...
    return data_subset #was data_sorted
...
def cos_sim_shift(original_emb, 
    ...
    cos = torch.nn.CosineSimilarity(dim=-1)##Changed to fix error message with dim=2
    ...
    cos_sims_df = label_embs((perturb_v_end-origin_v_end),downsampled_data,emb_label)
    #cos_sims_df = pd.DataFrame((perturb_v_end-origin_v_end).cpu().numpy())
    print(cos_sims_df.head())
    return [(perturb_v_end-origin_v_end).to("cpu")]
