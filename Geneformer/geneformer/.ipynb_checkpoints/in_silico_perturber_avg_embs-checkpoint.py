...
def remove_indices_from_emb(emb, indices_to_remove, gene_dim):
    ...
    if len(sliced_emb) == 2039:
        sliced_emb=sliced_emb[:-1]
    elif len(sliced_emb) == 2041:
        sliced_emb=sliced_emb[:-1]
    return sliced_emb
...
def remove_indices_from_emb_batch(emb_batch, list_of_indices_to_remove, gene_dim):
    ...
    output_batch = torch.stack([
                    ...
                    i, idxs in enumerate(list_of_indices_to_remove) #idxs = value of the index to remove
                    ])#the parameter for torch.stack is a generator containing a list of tensors.
    return output_batch

def label_embs(embs, downsampled_data, emb_labels):
    embs_df = pd.DataFrame(embs.cpu().numpy())
    if emb_labels is not None:
        for label in emb_labels:#gets cells states and labels for each cell in downsampled_data.
...
def quant_cos_sims(model, 
    ...
    for i in range(0, total_batch_length, forward_batch_size):
        ...
        if perturb_group == True:
            ...
            if len(tokens_to_perturb) == 1:
                ...
                for idx in indices_to_perturb[i:i+forward_batch_size]:#Made a change as this was i+max_range which caused errors
                    if idx == [-100]:
                        indices_to_perturb_minibatch.append(end_range)
                    elif len(idx) < num_perturbed:
                        indices_to_perturb_minibatch.append(idx + end_range[-(num_perturbed-len(idx)):])#changed this from -num_perturbed - gives dimensional mismatches.
                    else:
            ...
            #if original_minibatch_emb.size() != minibatch_emb.size():
                #indices to perturb minibatch is a list of lists
        ...
        if perturb_group == False:
            ...
            perturb_emb, original_emb = cos_sim_shift(original_emb, 
                                                        minibatch_emb, 
                                                        perturb_group,
                                                        output_directory,
                                                        output_prefix,
            ...
            perturb_embs, original_embs = cos_sim_shift(original_minibatch_emb,
                                                      minibatch_emb, 
                                                      perturb_group,
                                                      ...
                                                      downsampled_data,
                                                      emb_label)
        perturb_emb_list += [perturb_embs]
        original_emb_list += [original_embs]
        del outputs
        del minibatch_emb
    ...
    perturb_emb = torch.cat(perturb_emb_list)
    perturb_emb_df = pd.DataFrame(torch.squeeze(perturb_emb).cpu().numpy())
    perturb_df_embs_only = perturb_emb_df.iloc[:,:256]
    perturb_mean = perturb_df_embs_only.mean()
    
    original_emb = torch.cat(original_emb_list)
    original_emb_df = pd.DataFrame(torch.squeeze(original_emb).cpu().numpy())
    original_df_embs_only = original_emb_df.iloc[:,:256]
    original_mean = original_df_embs_only.mean()
    
    #euclidean_perturb_vs_original= (original_emb - perturb_emb).pow(2).sum(1).sqrt()
    
    return perturb_mean, original_mean, len(perturb_emb)
...
def cos_sim_shift(original_emb, 
    ...
    #if (perturb_v_end-origin_v_end).numel() == 1:#need a list of lists if there is only one element.
    #    return [([perturb_v_end-origin_v_end]).to("cpu")]
    #return [(perturb_v_end-origin_v_end).to("cpu")]
    return perturb_emb, original_emb
...
class InSilicoPerturberEmbsScreen:
    valid_option_dict = {
    ...
    def __init__(
        ...
        filter_state: None, string
            Default is to use all cell states for the in silico perturbation study.
            If true, will filter for the state specified in filter_state.
        cell_states_to_model: None, dict
        ...
        ref_emb : Series
            Reference average embedding from which Euclidean distances can be calculated.
        ensg_list : list
            List of Ensembl Gene IDs across which to perform the screen.
        min_genes : None, int
            Number of genes to require to be present in a cell when deleting a group.
            If None, will require every gene to be present.
        sample_cells : None, int,
            Number of cells from the larger dataset from which to sample.
            If None, will use the whole dataset.
        ...
        if self.genes_to_perturb == "all":
            ...
            self.tokens_to_filter = [self.gene_token_dict.get(gene) for gene in self.genes_to_perturb]
    ...
    def perturb_data(self, 
        ...
        state_embs_dict = None#remove this if doesn't work
         # filter for target cells
        state_to_filter = self.filter_state
        #this filters the filtered input data so that only cells of the chosen state are measured.
        def filter_for_origin(example):
            return example[state_name] in [state_to_filter[0]]

        if self.filter_state is not None:
            filtered_input_data = filtered_input_data.filter(filter_for_origin, num_proc=self.nproc)
            #cut here
        '''
        avg_embs_df = pd.DataFrame()
        gene_list = ["ENSG00000111596",#cnot2
        ...
        for gene in gene_list:
            self.tokens_to_perturb = [self.gene_token_dict.get(gene)] # ,self.tokens_to_filter[0]]- filters for cnot1 also
            #self.tokens_to_perturb = [self.gene_token_dict.get(gene)]
            perturb_avg_emb, original_avg_emb, n = self.in_silico_perturb(model,
                                          filtered_input_data,                          
                                          layer_to_quant,
                                          state_embs_dict,
                                          output_directory,
                                          output_prefix)
            if perturb_avg_emb is not None:
                perturb_euc_distance = np.linalg.norm(np.array(self.ref_emb['0'])-np.array(perturb_avg_emb))
                print(perturb_euc_distance)
                #orig_euc_distance = np.linalg.norm(np.array(self.ref_emb['0'])-np.array(original_avg_emb))
                row = perturb_avg_emb.to_frame().T
                #row = pd.DataFrame()
                row['gene'] = [gene]
                row['euclidean_distance'] = [perturb_euc_distance]
                row['n'] = [n]
                avg_embs_df = pd.concat([avg_embs_df, row],axis=0, ignore_index=True)
            else:
                with open('/mnt/scratchc/ghlab/toby/in_silico_perturb_data/avg_embs_vs_cnot1/missing_genes.txt','a') as file:
                    file.write(f'{gene}\n')
        avg_embs_df.to_csv(f'{output_directory}/proof_of_concept.csv')
        '''
        cds_present = pd.read_csv('/mnt/scratchc/ghlab/toby/in_silico_perturb_data/cds_codon_optimalities.csv')
        for i in range(self.gene_indices[0],self.gene_indices[1],100):
            avg_embs_df = pd.DataFrame()
            for gene in self.ensg_list[i:(i+100)]:
                #self.tokens_to_perturb = [self.gene_token_dict.get(gene),self.tokens_to_filter[0]] - filters for cnot1 also
                self.tokens_to_perturb = [self.gene_token_dict.get(gene)]
                print(gene)
                if gene in list(cds_present['ensembl_id']):
                    perturb_avg_emb, original_avg_emb, n = self.in_silico_perturb(model,
                                                  filtered_input_data,                          
                                                  layer_to_quant,
                                                  state_embs_dict,
                    ...
                    perturb_avg_emb = None
                if perturb_avg_emb is not None:
                    euc_distance = np.linalg.norm(np.array(self.ref_emb['0'])-np.array(perturb_avg_emb))
                    print(euc_distance)
                    row = perturb_avg_emb.to_frame().T
                    row['gene'] = gene
                    row['euclidean_distance'] = euc_distance
                    row['n'] = n
                    avg_embs_df = pd.concat([avg_embs_df, row],axis=0, ignore_index=True)
                else:
                    with open('/mnt/scratchc/ghlab/toby/in_silico_perturb_data/avg_embs_vs_cnot1/missing_genes.txt','a') as file:
                        file.write(f'{gene}\n')
            avg_embs_df.to_csv(f'{output_directory}unbiased_screen_{i}_{i+100}.csv')
    
    # determine effect of perturbation on other genes //downsampled data above was filtered_input_data 
    def in_silico_perturb(self,
        ...
        #remove second condition here to compare oex and del on the same cells
        if (self.tokens_to_perturb != "all") and (self.perturb_type != "overexpress"):
            ...
            if self.sample_cells:
                filtered_input_data = filtered_input_data.select([x for x in range(self.sample_cells)])
            filtered_input_data = filtered_input_data.filter(if_has_tokens_to_perturb, num_proc=self.nproc)
            if len(filtered_input_data) == 0:
                logger.error(
