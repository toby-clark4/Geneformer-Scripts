...
def downsample_and_sort(data_shuffled, max_ncells):
    ...
    # sort dataset with largest cell first to encounter any memory errors earlier (check here as well)
...
def remove_indices_from_emb_batch(emb_batch, list_of_indices_to_remove, gene_dim):
    print(f'{len(list_of_indices_to_remove)} is length of list_of_indices_to_remove')
...
def get_cell_state_avg_embs(model,
    ...
    for possible_state in possible_states:
        ...
        filtered_input_data_state = filtered_input_data.filter(filter_states, num_proc=num_proc) #problem is HERE
...
class InSilicoPerturberEmbs:
    ...
    def perturb_data(self, 
        ...
        if self.cell_states_to_model is None:
            ...
            #this filters the filtered input data so that only cells of the start state are measured.
